...
length(groups_a)
hc_a$ac
fviz_cluster(list(data = acsMain_svi_scaled, cluster = groups_a),main="Agglomerative Nesting Clustering Chart")
```
...
noclust <- subset(acsMain_svi_scaled, select= -c(cluster_diana, cluster_agnes, cluster_complete, cluster_ward))
...
agnes_model <- glm(covid_impact ~ ., data=acsMain_svi_scaled_AGNES)
summary(agnes_model)
```
