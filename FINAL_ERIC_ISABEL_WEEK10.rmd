...
#what is X 
acsMain_svi_scaled <- select(acsMain_svi_scaled, -c(X))
...
base_model <- lm(covid ~ ., data=noclust)
summary(base_model)
...
ward_model <- lm(covid ~ ., data=ward_dat)
summary(ward_model)
```
...
#ward_1 <- subset(ward_dat, cluster_ward==1)
complete_model <- lm(covid ~ ., data=complete_dat)
...
#ward_1 <- subset(ward_dat, cluster_ward==1)
agnes_model <- lm(covid ~ ., data=agnes_dat)
summary(agnes_model)
```
...
#ward_1 <- subset(ward_dat, cluster_ward==1)
diana_model <- lm(covid ~ ., data=diana_dat)
summary(diana_model)
